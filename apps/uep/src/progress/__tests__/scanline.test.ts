...
/* ── 迷霧閘門（S10-2 rush prevention）── */

const FOG_SCROLL_HEIGHT = 10000;
const FOG_CLIENT_HEIGHT = 1000;
// 門檻 = FOG_JUMP_THRESHOLD_VH(1.5) * 1000 / 10000 = 0.15

/**
 * 建立帶捲動容器的 DOM，並把每個標記釘在指定的 ratio 位置。
 * `computeElementRatio` 讀的是 getBoundingClientRect + scrollTop，
 * 這裡讓 scrollTop 維持 0，top 直接等於 ratio * scrollHeight。
 */
function buildFogDom(markerRatios: number[], flagsAt?: Record<number, string>) {
  ...
  Object.defineProperty(root, 'scrollHeight', {
    value: FOG_SCROLL_HEIGHT,
    configurable: true,
  });
  Object.defineProperty(root, 'clientHeight', {
    value: FOG_CLIENT_HEIGHT,
    configurable: true,
  });
  ...
  container.innerHTML = markerRatios
    .map((_, i) =>
      flagsAt?.[i]
        ? `<div data-role="${PROGRESS_MARKER_ROLE}" data-grants-flags="${flagsAt[i]}"></div>`
        : `<div data-role="${PROGRESS_MARKER_ROLE}"></div>`
    )
    .join('');
  Array.from(container.querySelectorAll('[data-role]')).forEach((el, i) => {
    el.getBoundingClientRect = () =>
      ({ top: markerRatios[i] * FOG_SCROLL_HEIGHT }) as DOMRect;
  });
...
describe('createScanline — 迷霧閘門', () => {
  it('連續推進的標記逐步把迷霧線往前推', async () => {
    const { createScanline, uepProgress } = await freshScanline();
    const { root, container, sentinel } = buildFogDom([0.05, 0.15, 0.28]);

    createScanline({ pageId: PAGE_ID, container, sentinel, root });
    const { markerIO } = getObservers();

    markerIO.trigger([markerIO.observed[0]]);
    expect(uepProgress.getState().fogRatio[PAGE_ID]).toBe(0.05);
    markerIO.trigger([markerIO.observed[1]]);
    expect(uepProgress.getState().fogRatio[PAGE_ID]).toBe(0.15);
    markerIO.trigger([markerIO.observed[2]]);
    expect(uepProgress.getState().fogRatio[PAGE_ID]).toBe(0.28);
  });
  ...
  it('rush 後退回迷霧線附近重新往下讀，被凍結的標記會正常觸發', async () => {
    const { createScanline, uepProgress } = await freshScanline();
    const { root, container, sentinel } = buildFogDom([0.05, 0.18, 0.3], {
      2: 'met:novia',
    });
    const passed: { index: number }[] = [];
    ...
    markerIO.trigger([markerIO.observed[2]]); // 一開始就跳到 30% → 擋
    ...
    markerIO.trigger([markerIO.observed[0]]);
    markerIO.trigger([markerIO.observed[1]]);
    markerIO.trigger([markerIO.observed[2]]); // 這次在可及範圍內
    expect(passed.map((p) => p.index)).toEqual([0, 1, 2]);
    expect(uepProgress.getState().flags).toContain('met:novia');
  });
  ...
  it('哨兵進視窗但迷霧沒推到底 → 不完成、不補授旗標', async () => {
    ...
    markerIO.trigger([markerIO.observed[0]]);
    sentinelIO.trigger([sentinel]); // 拖捲軸直接到底

    const state = uepProgress.getState();
    expect(state.completedPageIds).not.toContain(PAGE_ID);
    expect(state.flags).not.toContain('met:dell');
    expect(state.flags).not.toContain(`completed:${PAGE_ID}`);
    expect(passed).toHaveLength(1); // 只有第一個標記，沒有哨兵事件
  });

  it('迷霧推到底後哨兵才放行完成', async () => {
    const { createScanline, uepProgress } = await freshScanline();
    const { root, container, sentinel } = buildFogDom([0.05, 1]);
    ...
    // 標記在 1.0，但一次推不過去（0.05 → 1 超過門檻），照順序推到底。
    // 步長用整數乘出來，避免浮點累加誤差自己撞上門檻
    for (let step = 1; step <= 10; step += 1) {
      const ratio = Math.min(step * 0.1, 1);
      const el = markerIO.observed[1];
      el.getBoundingClientRect = () =>
        ({ top: ratio * FOG_SCROLL_HEIGHT }) as DOMRect;
      markerIO.trigger([el]);
    }
    expect(uepProgress.getState().fogRatio[PAGE_ID]).toBe(1);
  ...
  it('已完成過的頁面重讀時完全不受迷霧限制', async () => {
    ...
    const { root, container, sentinel } = buildFogDom([0.05, 0.95], {
      1: 'met:novia',
    });
    ...
    markerIO.trigger([markerIO.observed[1]]); // 直接跳到 95%
    expect(uepProgress.getState().flags).toContain('met:novia');
    // 豁免時不推進迷霧線（這頁已經沒有迷霧可推）
