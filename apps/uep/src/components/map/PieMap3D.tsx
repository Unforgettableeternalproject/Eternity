...
export default function PieMap3D({
  zones, size = 560, defaultTilt = 52, defaultYaw = -16,
  hoveredId, onHover, onPickIntro, onCenterClick,
  showHints = true, depth = 26, baseTone = 'auto',
}: PieMap3DProps) {
  const [tilt, setTilt] = useState(defaultTilt);
  const [yaw, setYaw] = useState(defaultYaw);
  const [drag, setDrag] = useState<{ x: number; y: number; yaw: number; tilt: number } | null>(null);
  ...
  const slices = useMemo(() => zones.map((z, i) => {
    const a0 = (i / 5) * 360 - 90;
    const a1 = a0 + 72;
    const mid = a0 + 36;
  ...
  const renderDisc = (opacity = 1, mode: 'top' | 'wall' = 'top') => (
    <svg viewBox="-50 -50 100 100" style={{
      ...
      {slices.map(({ z, a0, a1, mid }) => {
        ...
        const x0 = Math.cos(rad0) * r, y0 = Math.sin(rad0) * r;
        const x1 = Math.cos(rad1) * r, y1 = Math.sin(rad1) * r;
        const isHover = hoveredId === z.id;
        const dx = (mode === 'top' && isHover) ? Math.cos(mid * Math.PI / 180) * 2.4 : 0;
        const dy2 = (mode === 'top' && isHover) ? Math.sin(mid * Math.PI / 180) * 2.4 : 0;
        const fillOp = mode === 'top' ? (isHover ? 0.10 : 0.04) : 0.06;
        ...
        return (
          ...
          >
            <path d={`M0 0 L${x0} ${y0} A${r} ${r} 0 0 1 ${x1} ${y1} Z`}
              fill={z.main} fillOpacity={fillOp}
              stroke={z.main}
              strokeOpacity={isHover ? 1 : 0.6}
              strokeWidth={mode === 'top' ? (isHover ? 0.7 : 0.4) : 0.2}
              style={{ cursor: mode === 'top' ? 'pointer' : 'default', transition: 'all .3s var(--ease)' }}
            />
            {mode === 'top' && (
              <text
                x={Math.cos(mid * Math.PI / 180) * 30}
                y={Math.sin(mid * Math.PI / 180) * 30}
                textAnchor="middle" fontSize="4" fontWeight="600"
                fill={isHover ? z.main : 'var(--ink-title)'}
                fontFamily="var(--font-display)"
                style={{ pointerEvents: 'none', letterSpacing: '0.08em', transition: 'fill .3s var(--ease)' }}
              >
                {z.label}
              </text>
            )}
      ...
      {mode === 'top' && (
        <>
          <circle r="46.5" fill="none" stroke="var(--uep-gold)" strokeOpacity="0.45" strokeWidth="0.18" />
          <circle r="48" fill="none" stroke="var(--uep-gold)" strokeOpacity="0.18" strokeWidth="0.12" strokeDasharray="0.6 1.4" />
          {slices.map(({ z, a0 }) => {
          ...
          <g onPointerDown={(e) => e.stopPropagation()}
            ...
            <text textAnchor="middle" y="2.0" fontSize="5.4"
              fontFamily="var(--font-display)" fontWeight="600"
              fill="var(--uep-gold)">U</text>
          </g>
        </>
      )}
    </svg>
  ...
  return (
    <div style={{
      ...
      perspective: size * 2.6, perspectiveOrigin: '50% 28%',
    ...
    >
      {/* shadow oval */}
      <div style={{
        position: 'absolute', inset: '60% 10% -8% 10%',
      ...
      <div style={{
        ...
        <svg viewBox="-50 -50 100 100" style={{ width: '100%', height: '100%', animation: 'slow-rotate 60s linear infinite' }}>
          <circle r="49" fill="none" stroke={cardZone ? cardZone.main : 'var(--uep-gold)'}
            strokeWidth="0.18" strokeDasharray="0.4 1.2" strokeOpacity="0.7" />
          <circle r="51" fill="none" stroke={cardZone ? cardZone.main : 'var(--uep-gold)'}
            strokeWidth="0.12" strokeDasharray="0.2 2" strokeOpacity="0.5" />
        </svg>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <svg viewBox="-50 -50 100 100" style={{ width: '100%', height: '100%' }}>
            <circle r="46" fill="none" stroke="var(--uep-gold)" strokeOpacity="0.32" strokeWidth="0.4" />
            <circle r="46" fill={baseTone === 'light' ? '#FFF6E0' : 'rgba(20,21,26,0.55)'}
            ...
            {slices.map(({ z, a0 }) => {
              ...
              return z.glyphs?.map((g, gi) => {
                ...
                return (
                  <text key={`${z.id}-${gi}`}
                    x={gx} y={gy}
                    fontSize="3" fill={z.main}
                    fontFamily="var(--font-display)" textAnchor="middle"
                    style={{ opacity: 0, animation: `pie-drift 3.5s ${delay}s ease-out infinite` }}
                  >{g}</text>
                );
              });
            })}
      ...
      {cardZone && (
        <div style={{
          position: 'absolute',
          left: '50%', top: '50%',
          transform: `translate(${Math.cos(((cardIndex / 5) * 360 - 90 + 36 + yaw) * Math.PI / 180) * size * 0.46 - 110
            }px, ${Math.sin(((cardIndex / 5) * 360 - 90 + 36 + yaw) * Math.PI / 180) * size * 0.46 * Math.cos(tilt * Math.PI / 180) - 70
            }px)`,
          ...
          {(['tl', 'tr', 'bl', 'br'] as const).map(c => (
            <span key={c} style={{
              ...
              borderColor: cardZone.main, borderStyle: 'solid', borderWidth: 0,
          ...
          <div style={{
            ...
            color: cardZone.main, letterSpacing: '0.18em',
            textTransform: 'uppercase' as const,
          }}>{cardZone.kicker} · {cardZone.en}</div>
          <div style={{
            fontFamily: 'var(--font-display)', fontSize: 20,
            color: 'var(--ink-title)', fontWeight: 600, marginTop: 2,
          }}>{cardZone.label}</div>
          <div style={{ fontSize: 12, color: 'var(--ink-soft)', marginTop: 6, lineHeight: 1.55, fontFamily: 'var(--font-serif-tc)' }}>
            {cardZone.blurb}
          </div>
          <div style={{
            fontFamily: 'var(--font-mono)', fontSize: 10,
            color: 'var(--ink-mute)', marginTop: 10, paddingTop: 8,
      ...
      {showHints && !drag && (
        <>
          <div style={{
            ...
            textTransform: 'uppercase' as const, pointerEvents: 'none',
          }}>↻ 拖曳旋轉</div>
          <div style={{
