 ...
 *
 * 手動驗收浮島解鎖鏈的痛點：解鎖狀態埋在 ProgressState 裡，沒有反向操作
 * 入口——解鎖過就再也測不到解鎖儀式。這裡在 dev 提供
 * `window.__uepIslandsTest` 讓 console 直接操控。
 *
 * 與 `__uepLostBookmarkTest`（history 書籤專用）互補：這裡管的是
 * 全 zone 通用的解鎖狀態。掛載點在 IslandHost（全站覆蓋）。
 *
 * 2026-07-26 移除 `visit()` / `unvisit()` / `status().visitedZones`：
 * `zone:visited:*` 旗標本身已廢除（見 unlockRitual.ts）。它們原本存在
 * 就是為了繞過「旗標只在 ReaderShell mount 時授予」的時機陷阱——
 * 陷阱的根源拔掉後，補丁也不需要了。
 */
