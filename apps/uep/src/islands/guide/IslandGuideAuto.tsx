...
export default function IslandGuideAuto() {
  ...
  const eligible = useCallback((id: IslandId): boolean => {
    const p = progressRef.current;
    return (
      desktopRef.current &&
      canUseIslands(p) &&
      shouldMountIsland(p, id) &&
      !p.islandGuidesSeen.includes(id)
    );
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void (async () => {
        ...
        if (cancelled || !eligible(candidate)) {
        ...
        if (cancelled || !mounted || !eligible(candidate)) {
          schedulingRef.current = false;
          return;
        }
  ...
  }, [progress, desktop, active, eligible]);

  // overlay 顯示中失去資格（登出／切觀測者／縮成手機／停用該島）→ 收掉。
  // session key 已經寫了，本 session 不再自動打擾
  useEffect(() => {
    if (active && !eligible(active)) setActive(null);
  }, [active, progress, desktop, eligible]);
