 ...
 * 不快取（下次重試）且回 null——消費端降級為本頁求值，不劣於修前。
 */
...
const cache = new Map<string, Promise<ProgressTreeAdapter | null>>();

/** 取得指定 zone 的 tree-aware gating 求值器；失敗回 null（降級本頁求值） */
export function fetchZoneProgressTree(
  zone: string
): Promise<ProgressTreeAdapter | null> {
