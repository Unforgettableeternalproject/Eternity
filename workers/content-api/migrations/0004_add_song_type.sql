-- 新增 Echoes 區域頁面類型：cluster, subcategory, song
...
CREATE TABLE pages_new (
  ...
  page_type         TEXT NOT NULL DEFAULT 'page'
    CHECK (page_type IN ('zone','chapter','arc','section','page','cluster','subcategory','song'))
);
...
SELECT
  id, area, title, slug, sort_order, content,
  source_file, base_content_hash,
  COALESCE(status, 'synced'),
  metadata, created_at, updated_at, parent_id,
  COALESCE(depth, 0),
  COALESCE(page_type, 'page')
FROM pages;
