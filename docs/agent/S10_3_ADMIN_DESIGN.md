...
> ### ⚠️ 2026-07-30：D-1 反轉為「自動註冊，不擋存檔」
>
> 艾斯維爾在 T-A6 驗收時提出：旗標應該比照 `entityKey`／`storyKey`——「不是先註冊
> 才選，而是填了之後後註冊」。查證後這個論點成立，**強制註冊已移除**。
>
> 三個理由：
>
> 1. **與 key 的模式不一致**。`EntityKeyField` 是純自由輸入 input（無 picker、
>    無 datalist），只有格式與重複的即時警告，檔頭把理由寫死了：「校驗是即時
>    警告不阻擋輸入——entityKey 是語意資產，由設計者統一命名」。存檔時
>    `ensureInterlinkKeys` 建殼列，說明事後在 `/admin/keys` 補。旗標沒有理由
>    是唯一需要事前登記的識別碼。
> 2. **typo 已經抓得到**。T-A3 的巡查會讓它留下很好認的指紋：打錯的那個標
>    `unused`（有授予沒人要求）、正確的那個標 `orphan`（有要求沒人授予），
>    一組同時出現幾乎只有 typo 一種成因，「只顯示有問題的」直接撈得出來。
>    事前擋換來的是每次存檔都多一道手續。
> 3. **事前擋會關掉 derived 旗標的需求端**。gate 想要求「聽過某首歌」
>    （`{storyKey}:song`）或「看過某張圖」（`image:*`）時，那些旗標依設計
>    不可註冊，於是永遠填不進 `requiresFlags`。這個能力在強制版本下是死路。
>
> 現行語意：`upsertPage()` 呼叫 `ensureFlagsRegistered()` 把內容裡的自訂旗標
> 補進註冊表，回應帶 `autoRegisteredFlags`（與批次匯入路徑同一個欄位名）。
> derived 一律豁免。**不再有 409。**
>
> `FlagPicker` 相應改為「註冊表是建議清單不是白名單」：可選既有、可直接輸入
> 新名（存檔時自動註冊）、也可就地新建並順便填標籤（立即 POST）。
>
> 代價已知並接受：打錯字的旗標會靜默進註冊表。緩解是巡查清單，不是事前攔截。

（以下為原強制設計的論證，保留作為決策脈絡）

**2026-07-29 實測後曾改按「強制」設計**——正式 D1 的 244 頁裡，帶 `data-grants-flags` 的頁面是 **0**，設了 `requiresFlags` 的 5 頁要求的全是 `completed:*`（A 類自動旗標）。也就是說**全站目前一個自訂旗標都還沒有**，強制註冊的遷移成本是零，不存在「既有內容存不了檔」的問題。
...
| R5 | 旗標註冊表與內容的一致性沒有強制力 | 仍可能出現打錯字的旗標 | **這是 2026-07-30 D-1 反轉後刻意接受的代價**（見 §2-1）。緩解就是巡查：打錯的標 `unused`、正確的標 `orphan`，一組同時出現幾乎只有 typo 一種成因，`/admin/keys` 的「只顯示有問題的」直接撈得出來 |
...
| # | 問題 | 定案 | 影響 |
|---|---|---|---|
| **D-1** | 旗標註冊是否強制 | ~~強制~~ → **做註冊表但不強制**（2026-07-30 反轉，見 §2-1） | 艾斯維爾確認會用 FlagMarker 的頁內粒度（「讀到某段 → 解鎖某首歌」這類比整頁完成更細的觸發）。`uep_flags` 表 + `FlagPicker` 保留；**存檔 409 已移除**，改為 `ensureFlagsRegistered` 自動註冊，理由是與 `entityKey`／`storyKey` 的後註冊模式一致、typo 由巡查抓、事前擋會關掉 derived 旗標的需求端 |
